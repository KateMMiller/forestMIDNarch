...
#'  Plot_Name, Tree_Number_MIDN, Distance, Azimuth, Orientation, Status_ID (n=7), DBH, x, and y.
#'
...
treeMap<-function(df){
  ...
  p<-ggplot(data=df,aes(x=x, y=y, group=Status_ID, fill=Status_ID, size=DBH, label=Tree_Number_MIDN))+
    ...
    geom_jitter(aes(fill=Status_ID),shape=21, width=0.25)+ xlim(-11.7,11.7)+ylim(-11.7,11.7)+
