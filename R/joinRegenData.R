...
#' Stocking index thresholds are 2 for areas with low deer, and 8 for areas with high deer.
...
# Prepare the seedling data
  ...
  quad1 <- merge(park.plots, quadsamp[,1:14], by='Event_ID', all.x=T)
  ...
  saps3 <- merge(park.plots, saps2, by = c('Location_ID','Event_ID'), all.x = T)
  ...
  saps5 <- saps4 %>% group_by(Event_ID, TSN, Status_ID, Loc_Type) %>%
    summarise(Latin_Name = first(Latin_Name), Common = first(Common),
              Canopy_Exclusion = first(Canopy_Exclusion), Exotic = first(Exotic),
              sap.stems = sum(sap, na.rm=T),
              avg.sap.dbh = mean(DBH, na.rm=T)) %>% ungroup() %>%
