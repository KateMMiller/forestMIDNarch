...
#' MIDN forest backend databases. You must use the 32-bit version of R to work.
#'
...
importData<- function(type=c('DSN','file'), odbc='MIDNFVM', path=NA){
  ...
  assign("saps", dbReadTable(db,"tbl_Microplot_Sapling_Data_MIDN"),envir=.GlobalEnv)
  setTxtProgressBar(pb,10)
  assign("micro",dbReadTable(db,"tbl_Microplot_Characterization_Data"),envir=.GlobalEnv)
  setTxtProgressBar(pb,11)
  assign("sdlg",dbReadTable(db,"tbl_Quadrat_Seedling_Data"),envir=.GlobalEnv)
